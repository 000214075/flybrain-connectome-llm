...
**同一轮晚些时候的版本复核（20:45，`local-search` fetch `arxiv.org/abs/2606.17745v2`）**：仍是 **v2**（v1 2026-06-16、v2 2026-07-02），
**没有出版版链接**，单作者；标题、作者、abstract 与 §7.15 记的一致 ⇒ 上述引用**到 2026-09-19 20:45 仍然有效**。
**本项目随后真的做了那个零训练测量的一半**（报告 §7.45：度匹配重连已独立复核有效；冻结、同 seed 的 real-vs-shuffled
比较显示**粗略量几乎与接线无关**，与本文前半句一致；**空间路由那一半还没做**）。
